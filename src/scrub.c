...
int ma(int ac, char *av[])
{
     ...
     */
    prog = basename(av[0]);
    while ((c = GETOPT(ac, av, OPTIONS, longopts)) != EOF) {
        switch (c) {
        case 'v':   /* --version */
     ...
     */
    if (Xopt) {
        if (filetype(av[optind]) == FILE_NOEXIST) {
     ...
     */
        ...
        int i, errcount = 0;
        for (i = optind; i < ac; i++)
            errcount += scrub_object(av[i], &opt, true, false);
        if (errcount > 0) {
            fprintf (stderr, "%s: no files were scrubbed\n", prog);
            exit(1); 
        }
        for (i = optind; i < ac; i++) {
            if (scrub_object(av[i], &opt, false, nopt) > 0)
                exit(1);
        }
    /* Scrub single file/device.
     */
    } else {
        if (scrub_object(av[optind], &opt, false, nopt) > 0)
            exit(1);
    }
